  ...
  <div class="friends-main-container">
    ...
    <div class="friends-container">
      <div class="friends-inner-container">
        ...
        <div v-if="handleEmptyList()" class="photo-empty">
          <span class="comment" style="margin-bottom: 5rem;">
            Add Friends
          </span>
        </div>
        ...
        <div class="friends-list" v-if="!handleEmptyList()">
          ...
          >
            <div class="friend-main-content">
              <div class="user-images">
                ...
                <span class="userIcon-image">
                  <img
                    :src="handleImages(user.userName)"
                    class="profile-picture"
                    alt=""
                  />
                  <h5 @click="showUserProfile(user.userName, user.userId)">
                    {{ user.userName }}
                  </h5>
                  <button
                    class="btn btn-danger"
                    @click="handleUnfriend(user.userName)"
                  >
                    Unfriend
                  </button>
                </span>
          ...
          <div
            v-show="displayFollowers"
            v-for="user in $store.state.userData.followers"
            :key="user.userName"
            class="friend-content"
          >
            <div class="friend-main-content">
              <div class="user-images">
                ...
                <span class="userIcon-image">
                  ...
                  <button
                    class="btn btn-success unfriend-btn"
                    v-if="followingList.includes(user.userName)"
                    disabled
                  >
                    Following
                  </button>
                  <button
                    ...
                    v-else
                    @click="handleFollow(user.userName, 'follow')"
                  >
                    Follow Back
                  </button>
                </span>
          ...
          <div
            v-show="displayFollowing"
            v-for="user in $store.state.userData.following"
            :key="user.userName"
            class="friend-content"
          >
            <div class="friend-main-content">
              <div class="user-images">
                ...
                <span class="userIcon-image">
                  ...
                  <button
                    class="btn btn-danger unfriend-btn"
                    @click="handleFollow(user.userName, 'unfollow')"
                  >
                    unfollow
                  </button>
                </span>
        ...
        >
          <div class="friends-contents-list">
            ...
            <div class="request-buttons">
              <button
                class="btn btn-info"
                v-if="user.requestStatus !== 'Request Sent'"
  ...
  methods: {
    loadData() {
      ...
      const randomUsers = users.filter(
        (userName) =>
          ![...friendsList, this.userData.userName].includes(userName)
      );
      ...
      for (const userName in allUsers) {
        ...
        for (let index in allUsers[userName].requests) {
          if (
            allUsers[userName].requests[index].userName ===
      ...
      if (allUsers[this.userData.userName].requests.length) {
        for (let index in allUsers[this.userData.userName].requests) {
          allUsers[
            allUsers[this.userData.userName].requests[index].userName
  ...
  computed: {
    randomUsers() {
      ...
      const randomUsers = allUsers.filter(
        (user) => ![this.userData.userName, ...friends].includes(user.userName)
      );
