    ...
    <div class="posts-container">
      <div class="form-field mb-5" ref="formField">
        <span><img :src="userData.userThumbnail" alt="" />
          <span v-if="showUserName"> &nbsp; {{ userData.userName }}</span>
        </span>
        <form @submit.prevent="handlePublishPost()" ref="form">
          <textarea name="posttextarea" v-model="postTextArea" cols="30" rows="3" :class="postStyle"
            placeholder=" Make a post" @click="handleTextarea()" ref="textArea"></textarea>
      ...
      <section class="post-container" v-for="post in newsFeeds" :key="post.postId" :ref="post.userName">
        <div class="userImage" @click="showUserTimeline(post.userName, post.userId)">
          ...
          <span class="userName-Timeposted"><span>{{ post.userName }}</span><span> Posted {{ showDate(post.datePosted)
              }}</span>
          </span>
        </div>
...
<div class="post-Video"   v-if="post.postVideos.videoId.length"  @click="showit(post.postVideos)">
<video width="500" heigth="100" controls autoplay >
  ...
  Your browser does not support HTML video.
        ...
        <div class="ml-5 pt-2 comments" v-for="comment in post.comments" :key="comment.commentId">
          <div class="userImage ml-5" @click="showUserTimeline(comment.userName, comment.userId)">
            ...
            <span class="userName-Timeposted"><span>{{ comment.userName }}</span><span> commented {{
    ...
    <div class="container-sidebar-two" :style="sidebarListTwoMargin">
  ...
  export default {
    ...
    data() {
      return {
        ...
        sidebarListTwoMargin: "margin-top:1rem",
        newsFeed: {},
        postTextArea: "",
    ...
    methods: {
      ...
      handleDisplayFunctions() {
        if (this.$store.state.displayFunctions.navCollapsed) {
          ...
          this.sidebarListTwoMargin = "margin-top:-3rem"
          return this.sidebarListOneMargin
        }
        else {
          this.sidebarListOneMargin = "margin-top:2rem"
          this.sidebarListTwoMargin = "margin-top:1rem"
          return this.sidebarListOneMargin
        }
