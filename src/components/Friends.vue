<template>
  <div class="timeline-friends-container">
    ...
    <div class="empty-friens-list" v-if="!emptyFriendsList">
      <span class="names-Bold">{{ userName }} Does not Have Friends</span>
    </div>
    <div v-if="emptyFriendsList" class="friends-content-grid">
      ...
      >
        <div class="friend-main-content">
          <img
            @click="showUserProfile(friend.userName)"
            :src="handleCoverImage(friend.userName)"
            alt=""
            class="cover-image"
          />
          <img
            @click="showUserProfile(friend.userName)"
            :src="handleUserProfilePic(friend.userName)"
            alt=""
          ...
          <div class="user-name-details">
            <div>
              <h5 class="names-Bold" @click="showUserProfile(friend.userName)">
                {{ friend.userName }}
              </h5>
              <font-awesome-icon
                :icon="['fas', 'user-friends']"
                v-if="friend.requestStatus === 'friends'"
              />
              <button
                v-if="
                  friend.requestStatus !== 'friends' &&
                    friend.requestStatus !== 'you'
                "
                class="btn btn-info ml-2"
                @click="handleFriendRequest(friend, friend.requestStatus)"
  ...
  computed: {
    friendsLists() {
      ...
      for (const friend of this.userData.friends) {
        friendsList = {
          ...friendsList,
          [friend.userName]: { ...this.$store.state.allUsers[friend.userName] },
        };
      }

      this.friendsList = friendsList;
      Object.keys(this.friendsList).length > 0
        ? (this.emptyFriendsList = true)
        : (this.emptyFriendsList = false);

      let userfriendsList = this.$store.state.userData.friends.map(
        (user) => user.userName
      );

      let userRequestList = this.$store.state.allUsers[
        this.$store.state.userData.userName
      ].requests.map((userRequest) => userRequest.userName);

      for (let userName in friendsList) {
        let friendRequestList = this.$store.state.allUsers[
          userName
        ].requests.map((userRequest) => userRequest.userName);
        ...
        } else if (userName === this.$store.state.userData.userName) {
